...
const Product = class Product {
     ...
     */
     static inStock = (products) => {return products.filter(obj => obj.availability==='In Stock')};
     ...
     * Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array 
     */
    static halfOff = (products) => {return products.map(obj => new Product(`${obj.name},${obj.price/2},${obj.availability}`))}
     ...
     */
    static printProducts = (products) => {return products.forEach(obj =>  console.log(`Product: ${obj.name}, Cost: ${new Intl.NumberFormat('en-EN', { style: 'currency', currency: 'USD' }).format(obj.price)}, Availability: ${obj.availability==="In Stock"? "Yes" : "No"}`))}
